...
| Security policy controls | `renderSettings()`, `requiresKycBeforeSharing()`, `requiresWalletForBlockchainActions()` | Existing: `GET /api/security-policy`; backend wallet auth; optional `REQUIRE_KYC_BEFORE_SHARING` enforcement on grant sync | Keep real-mode wallet control locked on; keep KYC derived from backend policy; keep upload progress local only | `GET /api/security-policy`, `POST /api/assets/:assetId/access/grant-sync` | `showProgress` only. Security requirements are not client preferences. |
...
| Share policy: require KYC before sharing | `window.openShare()` uses backend policy in real mode and demo preference only in demo mode | Existing: `UserModel.kycStatus`; `REQUIRE_KYC_BEFORE_SHARING` blocks grant sync unless owner is `VERIFIED` | Replace mock KYC provider later; keep enforcement server-side | `GET /api/security-policy`, `POST /api/assets/:assetId/access/grant-sync` | No in real mode. |
| Share policy: require wallet before blockchain actions | `window.openShare()` uses locked real-mode policy | Existing auth requires wallet signature and real sync routes require `requireAuth` | Continue using authenticated backend session and MetaMask signatures | `GET /api/auth/me`, grant/revoke/version sync routes | No. |
